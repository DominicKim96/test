...
for(let i=0; i<menu_li_tit.length; i++) {
    menu_li_tit[i].addEventListener('click', function(){
        if(this.nextElementSibling != null) {
            let menu_li_sec_h = 0;
            if(this.parentElement.offsetHeight <= 37)
                menu_li_sec_h = this.nextElementSibling.offsetHeight;

            this.parentElement.style.height = 35 + menu_li_sec_h +"px"
        }
    })
}
...
///////////////// 하우스 도산
    ...
    //////////////////// 하우스 도산
            ...
            let dosan_menu = `<li class="product_pan_li ${i==0?'product_pan_li_chk':''}">
            ...
            let dosan_img =`<div class="item_img ${i==0?'item_opacity':''}">
                                    <img src="../product/dosan/${ITEM_LIST[0][i].src}" alt="">
                            </div>`
...
/////////////////// 신사
        ...
        let sinsa_img =`<div class="item_img ${i==0?'item_opacity':''}">
                            <img src="../product/sinsa/${ITEM_LIST[1][i].src}" alt="">
                        </div>`
...
let item_img_sec = document.getElementsByClassName('item_img_sec');
        ...
        for(let i=0; i < menu_txt_li.length; i++) {
            ...
            for(let j=0; j < menu_txt_li.length; j++) {
                ...
                product_pan_li[0].classList.add('product_pan_li_chk')
            })
        }
